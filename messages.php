<?php 
	...
	$sql = mysqli_query($mysqli,"SELECT SQL_CALC_FOUND_ROWS * FROM  users, messages WHERE messages.message_from = users.user_id AND messages.message_to = $session_user_id AND (username LIKE '%$q%' OR messages.message LIKE '%$q%') LIMIT $record_from, $record_to");
